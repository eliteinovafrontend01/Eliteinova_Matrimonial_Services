// src/components/admin/vendors/categories/DecorationsPage.jsx
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6">
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead><tr className="bg-gray-50">{['Vendor ID', 'Decorator Name', 'Specialization', 'Location', 'Rating', 'Bookings', 'Status', 'Verified', 'Actions'].map(h => <th key={h} className="px-4 py-3 text-left text-[11px] font-bold text-gray-400">{h}</th>)}</tr></thead>
          <tbody>
...
export const DecorationsPage = () => {
  ...
  const stats = [
    { label: 'Total Decorators', value: '48', icon: '🎊', color: 'border-purple-400', filter: 'All' },
    { label: 'Active', value: '37', icon: '✅', color: 'border-green-400', filter: 'Active' },
    { label: 'Pending Approval', value: '7', icon: '⏳', color: 'border-amber-400', filter: 'Pending' },
    { label: 'Theme Specialists', value: '19', icon: '🎨', color: 'border-pink-400', filter: 'Top Rated' },
  ];
  ...
  return (
    <div>
      <div className="rounded-2xl p-5 mb-6 bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200">
        <div className="flex items-center gap-4"><div className="text-4xl">🎊</div><div><h3 className="text-lg font-bold text-gray-800">Decorations</h3><p className="text-sm text-gray-500">Wedding Stage, Floral Décor, Mandap Setup, Reception Themes & Custom Concepts</p></div></div>
      </div>
      ...
      <VendorTable vendors={filteredVendors} title="All Decoration Vendors" count={`${filteredVendors.length} vendors shown`} activeFilter={activeFilter} onFilter={setActiveFilter} />
